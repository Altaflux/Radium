...
class AssignmentStatementVisitorTest extends Specification {
    ...
    def "should create Assignment Object from antlr generated AssignmentContext object"() {
        ...
        when:
        def assignment = new AssignmentStatementVisitor(expressionVisitor, fScope).visitAssignment(assignmentContext)
        then:
        1 * fScope.isLocalVariableExists(name) >> true
        2 * fScope.getLocalVariable(name) >> localVariable
